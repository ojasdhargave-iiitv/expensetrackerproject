...
void sorting()
{
    ...
    int count = 0;
    int check;
    printf("enter 1 for sorting by category\nenter 2 for sorting by amount\n");
    scanf("%d", &check);
    ...
    if (check == 1)
    {
        qsort(exparr, count, sizeof(menu), compareCategory); // Print sorted expenses
    ...
    {
        qsort(exparr, count, sizeof(menu), compareAmount); // Print sorted expenses
        printf("\n--- Sorted Expenses by Amount ---\n");
        for (int i = 0; i < count; i++)
        {
            printf("\nDate: %d\n Title: %s\n Category: %s\n Amount: %.2f\n", exparr[i].Date, exparr[i].Title, exparr[i].Category, exparr[i].Amount);
        }
